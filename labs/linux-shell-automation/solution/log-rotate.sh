...
# --- core logic -------------------------------------------------------------
# -mtime +N matches files modified strictly more than N*24h ago. We exclude
# *.gz so we never double-compress, and use -print0 / read -d '' so filenames
# with spaces or newlines are handled safely.
rotated=0
while IFS= read -r -d '' f; do
  if ((DRY_RUN)); then
    ...
    gzip -- "$f"
    log "INFO" "gzipped: ${f}.gz"
  fi
  rotated=$((rotated + 1))
done < <(find "$DIR" -type f ! -name '*.gz' -mtime "+${DAYS}" -print0)
