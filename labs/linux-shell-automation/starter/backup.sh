...
# TODO(student): create a gzipped tarball at "$archive" containing the source
# directory. Use `tar -czf "$archive" -C "$parent" "$name"` so paths inside the
# archive are relative ("$name/...") rather than absolute.
# tar ...   <-- implement this line
...
pruned=0
# TODO(student): if there are MORE than KEEP archives, delete the excess. The
# `archives` array is sorted newest-first, so everything from index KEEP onward
# is "old" and should be removed. Use the slice "${archives[@]:KEEP}" and rm -f
# each one under "$DEST". Increment `pruned` per deletion and log it.
if false; then   # TODO(student): replace condition; iterate and remove old archives
  :
fi
