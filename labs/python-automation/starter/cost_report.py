...
def summarize(rows: Iterable[Mapping[str, object]]) -> dict:
    ...
    """
    # TODO(student): Group rows by service and total them.
    #   1. Build `totals: dict[str, float]`. For each row, use parse_row(row) to
    #      get (service, amount), then ACCUMULATE (the same service may appear in
    #      several rows -- add, never overwrite).
    #   2. Build `ordered`: totals.items() sorted by amount DESCENDING, breaking
    #      ties by service name ASCENDING. (Hint: key=lambda kv: (-kv[1], kv[0]).)
    #   3. Build `services`: [{"service": s, "amount": round(a, 2)} for s, a in ordered].
    #   4. Build `grand`: round(sum of all amounts, 2).
    totals: dict[str, float] = {}
    # ... your accumulation loop here ...
    ordered: list = []           # replace
    services: list = []          # replace
    grand: float = 0.0           # replace
